...
countries = pd.DataFrame([{"id": i + 1, "name": fake.country(), "iso_code": fake.country_code()} for i in range(3)])
states = pd.DataFrame([{"id": i + 1, "name": fake.state(), "region_id": random.choice(regions["id"])} for i in range(5)])
...
categories = pd.DataFrame([{"id": i + 1, "name": f"Category {i + 1}"} for i in range(3)])
...
carriers = pd.DataFrame([{"id": i + 1, "name": name} for i, name in enumerate(["fedex", "amazon", "ups"])])
